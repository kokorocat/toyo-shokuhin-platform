    ...
    <div className="mx-auto min-h-screen max-w-4xl px-4 py-6">
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        {(() => {
          ...
          const masterReason = !masterSys
            ? "未設定"
            : masterSys.status !== "active"
              ? "停止中"
              : !masterHref
                ? "会社管理者以上の権限が必要です"
                : "";
          ...
          return masterDisabled ? (
            <div className="flex cursor-not-allowed items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 opacity-50">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-200 text-slate-400">
                {SYSTEM_ICONS.store_master}
              </div>
              <div>
                <p className="text-sm font-semibold text-slate-500">店舗・従業員マスター</p>
                <p className="text-xs text-slate-400">{masterReason}</p>
              </div>
            </div>
          ) : (
            <Link
            ...
            >
              ...
              <p className="text-sm font-semibold text-slate-800">店舗・従業員マスター</p>
            </Link>
          );
        })()}
      </section>
