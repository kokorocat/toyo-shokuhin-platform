      ...
      <form action={kioskSubmitInspection} className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-4 px-5 py-5">
            <div>
              <label htmlFor="store_manager_name" className="mb-1.5 block text-base font-medium text-slate-700">
                店長名（任意）
              </label>
              <input
            ...
            <div>
              <label htmlFor="hygiene_officer_name" className="mb-1.5 block text-base font-medium text-slate-700">
                食品衛生責任者名（任意）
              </label>
              <input
            ...
            <div>
              <label htmlFor="area_manager_name" className="mb-1.5 block text-base font-medium text-slate-700">
                エリア長名（任意）
              </label>
              <input
            ...
            <div>
              <label htmlFor="area_hygiene_officer_name" className="mb-1.5 block text-base font-medium text-slate-700">
                エリア衛生担当者（任意）
              </label>
              <input
            ...
            <div>
              <label htmlFor="implementer_name" className="mb-1.5 block text-base font-medium text-slate-700">
                実施者名
                <span className="ml-1 text-sm text-red-600">必須</span>
              </label>
              <input
                id="implementer_name"
          ...
          {INSPECTION_CATEGORIES.map((category) => (
            <div key={category.no} className="rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
                <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-800 text-sm font-bold text-white">
                  {category.no}
                </span>
              ...
              <div className="divide-y divide-slate-100">
                {category.items.map((q) => (
                  <fieldset key={q.code} className="px-5 py-5">
                    <legend className="mb-3 text-base font-medium text-slate-800">
                      {category.items.length > 1 && (
                        <span className="mr-1.5 text-sm font-semibold text-slate-400">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <label className="cursor-pointer rounded-lg border-2 border-slate-300 px-3 py-4 text-center text-base text-slate-700 transition-colors has-[:checked]:border-amber-600 has-[:checked]:bg-amber-50 has-[:checked]:font-bold has-[:checked]:text-amber-700">
                        <input
                          type="radio"
                          name={`answer_${q.code}`}
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4">
            <h2 className="text-base font-bold text-slate-900">改善が必要な項目がある場合の詳細</h2>
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4">
            <h2 className="text-base font-bold text-slate-900">自主評価</h2>
          </div>
          <div className="space-y-4 px-5 py-5">
            <fieldset>
              <legend className="mb-2 text-base font-medium text-slate-700">
                今月の総合的な自己評価
                <span className="ml-1 text-sm text-red-600">必須</span>
              </legend>
              <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label htmlFor="special_notes" className="mb-1.5 block text-base font-medium text-slate-700">
                特記事項（任意）
              </label>
              <textarea
                id="special_notes"
