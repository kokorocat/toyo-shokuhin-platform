...
export default async function RecipeApprovalsPage({
  ...
  const pendingApplications = groupByApplication(recentRows).sort((a, b) => {
    ...
    if (aDone !== bDone) return aDone ? 1 : -1; // 未判定を含むバッチを先に表示
  ...
  return (
    <div className="mx-auto min-h-screen max-w-5xl px-4 py-6">
      <PageHeader
        ...
        backLabel="レシピ一覧に戻る"
        title="レシピ承認"
        subtitle={`承認待ち ${totalPending}件`}
      />

      <div className="mb-5 flex justify-end gap-3">
        <Link
          href="/recipe/admin/approvals/history"
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 shadow-sm transition-colors hover:bg-slate-50"
        >
          承認履歴を見る
        </Link>
        <Link
                        ...
                        <div key={item.id} className="px-5 py-4">
                          ...
                          <div className="mt-3 flex flex-wrap items-start gap-3">
                            ...
                            <form action={approveRecipeWithoutPublishing}>
                              <input type="hidden" name="recipe_id" value={item.id} />
                              <SubmitButton
                                className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50"
                                pendingText="処理中..."
                              >
                                承認（未公開のまま）
                              </SubmitButton>
